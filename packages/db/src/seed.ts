/**
 * System-owned reference data: agencies, the tag taxonomy, standard gas mixes,
 * and a starter region tree.
 *
 * IDs are UUIDv5 derived from a fixed namespace, so re-running this updates
 ...
 *
 * This lives in `src` rather than in `prisma/` so it compiles to `dist` and
 * can run inside the API image. `pnpm deploy --prod` lands the package at
 * node_modules/@mydivelog/db, and Node refuses to strip types from TypeScript
 * under node_modules — so a `prisma/seed.ts` can never run on a deployed
 * machine, which is why no deployed environment had a tag taxonomy at all.
 */
...
export type SeedCounts = { agencies: number; tags: number; gasMixes: number; regions: number };
